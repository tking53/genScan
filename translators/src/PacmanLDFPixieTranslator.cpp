...
PacmanLDFPixieTranslator::PacmanLDFPixieTranslator(const std::string& log,const std::string& translatorname) : Translator(log,translatorname){
...
Translator::TRANSLATORSTATE PacmanLDFPixieTranslator::Parse(boost::container::devector<PhysicsData>& RawEvents){
...
int PacmanLDFPixieTranslator::ParseDirBuffer(){
...
int PacmanLDFPixieTranslator::ParseHeadBuffer(){
	this->CurrentFile.read(reinterpret_cast<char*>(&(this->check_bufftype)),sizeof(unsigned int));
	this->CurrentFile.read(reinterpret_cast<char*>(&(this->check_buffsize)),sizeof(unsigned int));
...
int PacmanLDFPixieTranslator::ParseDataBuffer(unsigned int& nBytes,bool& full_spill,bool& bad_spill){
	...
	while( true ){
		...
		if( this->CurrDataBuff.buffhead == HRIBF_TYPES::ENDFILE ){
			if( this->CurrDataBuff.nextbuffhead == HRIBF_TYPES::ENDFILE ){
				...
				if( not this->CurrentFile.eof() ){
					this->console->info("Read double EOF, but not at end of file yet, force reading until we get to the end");
					this->ReadNextBuffer(true);
				}
				return 2;
			}else{
...
int PacmanLDFPixieTranslator::ReadNextBuffer(bool force){
...
int PacmanLDFPixieTranslator::UnpackData(unsigned int& nBytes,bool& full_spill,bool& bad_spill,std::vector<bool>& entriesread){
	...
	std::vector<int> NumRead(this->CustomLeftovers.size(),0);
	...
	if( bad_spill ){
		for( size_t ii = 0; ii < NumRead.size(); ++ii ){
			for( int jj = 0; jj < NumRead[ii]; ++jj ){
				--(this->EvtSpillCounter[this->CustomLeftovers[ii].back().GetSpillID()%this->NUMCONCURRENTSPILLS]);
				this->CustomLeftovers[ii].pop_back();
			}
		}
	}
	return 0;
...
int PacmanLDFPixieTranslator::CountBuffersWithData() const{
