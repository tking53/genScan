...
int main(int argc, char* argv[]) {
	...
	try {
		...
		std::map<std::string, PLOTS::HisHelper2D> His2D = {
			...
			{"Ion_AnodeSum_v_X", {1000, -5, 5, 10000, 0, 1000000}},
			{"Ion_AnodeSum_v_Y", {1000, -5, 5, 10000, 0, 1000000}},
		...
		for (Long64_t ii = 0; ii < num_entries; ++ii) {
			...
			if (hasbeta) {
				...
				ValidSegments.push_back({*(Segment[0]), *(Segment[1]), *(Segment[2]), *(Segment[3]), *(Segment[4]), *(Segment[5]),
