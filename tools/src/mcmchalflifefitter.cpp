...
class BkgTerm {
public:
	BkgTerm(const std::vector<LimitedValue<T>>& p)
		: pars(p) {
	}
...
public:
	FlatBkg(const std::vector<LimitedValue<T>>& p)
		: BkgTerm<T>(p) {
	}
...
public:
	SlopeBkg(const std::vector<LimitedValue<T>>& p)
		: BkgTerm<T>(p) {
	}
...
public:
	ExpCuspBkg(const std::vector<LimitedValue<T>>& p)
		: BkgTerm<T>(p) {
	}
...
public:
	DecayCurve() = default;
	DecayCurve(const std::vector<T>& x, const std::vector<T>& y)
		: xvals(x)
		, yvals(y) {
	}
	DecayCurve(std::vector<T>&& x, std::vector<T>&& y)
		: xvals(std::move(x))
		, yvals(std::move(y)) {
	}
	DecayCurve(const std::vector<T>& x, const BkgTerm<T>& func)
		: xvals(x) {
