...
struct TH1B {
	...
	TH1B(const std::string& n, const std::string& t, int nb, float xl, float xh)
		: name(n)
		, title(t)
		, nbinsx(nb)
		, xlow(xl)
...
struct TH2B {
	...
	TH2B(const std::string& n, const std::string& t, int nbx, float xl, float xh, int nby, float yl, float yh)
		: name(n)
		, title(t)
		, nbinsx(nbx)
		, xlow(xl)
...
int main(int argc, char* argv[]) {
	...
	cmdline_options.add_options()("help,h", "produce help message")("numval,n", boost::program_options::value<unsigned int>(&MaxNumErg)->default_value(1000000), "number of iterations for the fill command");
