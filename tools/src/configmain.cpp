...
int main(int argc, char* argv[]) {
	...
	cmdline_options.add_options()("help,h", "produce help message notable regex patterns All Even Numbers : ^\\d*[02468]$ All Odd Numbers : ^\\d*[13579]$")("authorinfo,i", boost::program_options::value<std::string>(&authorinfo)->default_value("Author:GenConfig;Date:Today;Email:default AT gmail DOT com"), "Author info to generate, follow format we split on \";\"")("description,d", boost::program_options::value<std::string>(&description)->default_value("Automatically Generated xml by GenConfig"), "Description to insert into xml")("eventbuild,e", boost::program_options::value<std::string>(&eventbuild)->default_value("EventWidth:500;EventWidthUnit:ns;CorrelationType:rolling-trigger"), "Global info to generate, follow format we split on \";\"")("outputfile,o", boost::program_options::value<std::string>(&outputfile)->default_value("config.xml"), "name of config file to dump to")("numcrate,x", boost::program_options::value<int>(&numcrate)->default_value(1), "number of crates in default xml")("nummodule,y", boost::program_options::value<int>(&nummodule)->default_value(13), "number of modules per crate in default xml")("numchannel,z", boost::program_options::value<int>(&numchannel)->default_value(16), "number of channels per module in default xml")("processor,p", boost::program_options::value<std::vector<std::string>>(&processorlist), "name of processor to include [Allowed Multiple times]")("analyzer,a", boost::program_options::value<std::vector<std::string>>(&analyzerlist), "name of analyzer to include [Allowed Multiple times]")("cmapregex,r", boost::program_options::value<std::vector<std::string>>(&cmapregexlist), "how to populate the type:subtype:group:tags for each channel format is \"type:subtype:group:tags:[crate]:[module]:[channels]\" [Allowed Multiple times] example regex -> mtas:center:1:front:[1]:[0-4]:[0,2,4,6,8,10] defines MTAS's center front ring")("moduleregex,m", boost::program_options::value<std::vector<std::string>>(&moduleregexlist), "how to populate the type:subtype:group:tags for each channel format is \"Revision:Frequency:Firmware:TraceDelay:[crate]:[module]\" [Allowed Multiple times] example regex -> F:250:R42950:264:[0-12]:[0-2] defines the first three modules in all crates use RevF-250 with a 264ns trace delay")("counter,c", boost::program_options::value<std::vector<std::string>>(&counterlist), "replace the value in a regex with an incrementing value VARNAME:default_value only implemented for cmapregex on the subtype, group, and tags");
