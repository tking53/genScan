...
YAML::Emitter& operator<<(YAML::Emitter& out, const PeakFitter1D* pf) {
	...
	for (const auto& kv : pf->bvalues) {
		out << YAML::Key << kv.first << YAML::BeginMap << YAML::Key << "Min" << YAML::Value << kv.second.first << YAML::Key << "Max" << YAML::Value << kv.second.second << YAML::EndMap;
	}
	out << YAML::EndMap;
	out << YAML::Key << "Fixed" << YAML::BeginMap;
	out << YAML::Key << "NValues" << YAML::Value << pf->fvalues.size();
	for (const auto& kv : pf->fvalues) {
		out << YAML::Key << kv.first << YAML::BeginMap << YAML::Key << "Value" << YAML::Value << kv.second << YAML::EndMap;
	}
	out << YAML::EndMap;
