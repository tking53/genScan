	...
	int decode_next_hit() {
		...
		if (this->input.eof() or not this->input.good()) {
			if (not this->cycle_file()) {
				// need to swap input files
				this->decode_header();
				// recursive testing of trying to decode???
				this->decode_next_hit();
				return 0;
			} else {
				return -1;
			}
		} else {
			if (not input.read(reinterpret_cast<char*>(&board), sizeof(uint16_t))) {
				// we can't read the board, we're at the end of the file but not triggered the
				// eof/bad bit
				if (not this->cycle_file()) {
					// need to swap input files
					this->decode_header();
...
int main(int argc, char* argv[]) {
	...
	std::regex pattern("DataR_CH(\\d+)@(V|DT)(\\d+_?\\d+)_(.*)");
