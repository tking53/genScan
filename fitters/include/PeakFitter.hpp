...
struct PeakFitter2D : public PeakFitter {
	...
	PeakFitter2D(double xl, double xu, double yl, double yu, bool chi2, bool dbg, int mode, TH2* hist,
		...
		: PeakFitter(chi2, dbg, fixedvalues, boundedvalues)
		, XFitRange(xl, xu)
		, YFitRange(yl, yu)
		, fithist(hist) {
...
struct PeakFitter1D : public PeakFitter {
	...
	void InitSingleDaughterImplantationCurveFit() {
		...
		double amp = this->fithist->GetBinContent(zerobin + 1);
		double pn = 0.0;
		this->keys = {{"Constant", {0, bkg}}, {"Amplitude", {1, amp}}, {"HalfLife", {2, half_life}}, {"Pn", {3, pn}}, {"BetaHalfLife", {4, half_life}}, {"BetaNHalfLife", {5, half_life}}};
		AssignFitParNames();
		VerifyFixedValues();
		VerifyBoundedValues();
	...
	void InitNGaussNFit() {
		...
		VerifyBoundedValues();
		for (int ii = 0; ii < npeaks; ++ii) {
			if (this->fvalues.find("Mean" + std::to_string(ii)) == this->fvalues.end() and this->bvalues.find("Mean" + std::to_string(ii)) == this->bvalues.end()) {
		...
		if (integral > 0.0) {
			...
			if (not fitresult->IsEmpty()) {
				...
				for (int ii = 0; ii < npeaks; ++ii) {
					...
					this->components.at(2 * ii + 1)->SetParameters(this->Results["ComptonArea" + std::to_string(ii)], this->Results["Mean" + std::to_string(ii)], this->Results["Sigma" + std::to_string(ii)]);
