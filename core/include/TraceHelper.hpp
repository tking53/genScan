...
public:
	...
	TraceHelper(const unsigned int len)
		: data(std::vector<T>(len)) {
	}

	TraceHelper(const std::vector<T>& val)
		: data(val) {
	}

	TraceHelper(std::initializer_list<T> val)
		: data(val) {
	}

	TraceHelper(const TraceHelper& other)
		: data(other.data)
	...
	TraceHelper(TraceHelper&& other) noexcept
